...
class BannerController {
  ...
  @PostMapping("/{id}/banner/upload-url") @PreAuthorize("hasRole('ADMIN')") UploadUrl createUpload(@PathVariable Long id,@RequestParam(defaultValue="image/jpeg") String contentType){
  ...
  @PutMapping("/{id}/banner") @PreAuthorize("hasRole('ADMIN')") Event attach(@PathVariable Long id,@RequestBody BannerUpdate body){Event e=events.findById(id).orElseThrow(()->new ResponseStatusException(HttpStatus.NOT_FOUND));if(body.url()==null||body.url().isBlank())throw new ResponseStatusException(HttpStatus.BAD_REQUEST,"Banner URL is required");e.bannerUrl=body.url();return events.save(e);}
