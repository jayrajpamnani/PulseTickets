...
export class AppComponent {
  ...
  load() {
    this.http.get<any>(`/api/events?keyword=${encodeURIComponent(this.keyword)}`).subscribe({
      ...
      error: () => this.error.set('The API is unavailable. Start the local stack and refresh.')
    });
  }
  ...
  private signInDemo(credentials: { username: string; password: string }) {
