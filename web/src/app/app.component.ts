  ...
  template: `
    <header><b>PulseTickets</b><span>Discover memorable events</span>
      <ng-container *ngIf="!token()">
        <button (click)="openAuth('login')">Sign in</button>
        <button class="outline" (click)="openAuth('register')">Register</button>
      </ng-container>
      <ng-container *ngIf="token()">
    ...
    <div class="modal-backdrop" *ngIf="authOpen()" (click)="closeAuth()"><section class="auth-modal" (click)="$event.stopPropagation()">
      ...
      <p class="hint">{{ authMode === 'login' ? 'Sign in to manage and reserve tickets.' : 'Create an account to reserve tickets.' }}</p>
...
export class AppComponent {
  ...
  openAuth(mode: 'login'|'register' = 'login') { this.authMode = mode; this.authError.set(''); this.authOpen.set(true); }
  ...
  submitAuth() {
    ...
    request.subscribe({ next: r => { localStorage.setItem('token', r.token); this.token.set(r.token); this.authBusy.set(false); this.closeAuth(); }, error: (e: HttpErrorResponse) => { this.authBusy.set(false); this.authError.set(e.status === 409 ? 'That username or email is already registered.' : e.status === 401 ? 'Incorrect username or password.' : (typeof e.error === 'string' ? e.error : e.error?.message || 'Please check your details and try again.')); } });
  }
  reserve(event: Event) {
    if (!this.token()) { this.authError.set(''); this.openAuth('login'); return; }
    this.http.post('/api/reservations', {}, { params: { eventId: event.id, quantity: 1 } }).subscribe({
      next: () => { this.error.set('Reservation confirmed!'); this.load(); },
      error: (e: HttpErrorResponse) => {
        if (e.status === 401) { this.signOut(); this.openAuth('login'); }
        else {
          const msg = typeof e.error === 'string' ? e.error : e.error?.message;
          this.error.set(msg || 'Reservation could not be created. Please try again.');
        }
      }
    });
  }
